...
To illustrate the data flow better and reason about it, \cref{buftypes} shows all buffers used on GPU.
...
\begin{table}[]
	...
	\begin{tabular}{@{}lll@{}} \toprule
		...
		Normalized deformed subregions  &      &                    \\
		Normalized reference subregions &      &                    \\
		Cross--correlated subregions    &      &                    \\
		Positions of maxima             &      &                    \\
		Neighborhoods of maxima         &      &                    \\ \bottomrule
	\end{tabular}
...
\Cref{overview} also shows which parts are done on CPU and which on GPU. There are 3 parts separated by data transfer between the main and the GPU memory: CPU first loads a pattern, GPU processes it, and then CPU does the finalization of results. It is desirable that we parallelize those tasks, so that GPU is fully utilized and does not wait for the CPU.
