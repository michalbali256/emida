...
inline std::vector<vec2<T>> get_offset(T* pic, T* temp, size_t cols, size_t rows, size_t b_size)
{
	...
	auto hann_y = generate_hanning<T>(rows); TICK("Generate hanning: ");
	...
	CUCH(cudaMalloc(&cu_cross_res, cross_size * b_size * sizeof(T))); TICK("Cross result malloc: ");

	run_cross_corr(cu_temp, cu_pic, cu_cross_res, { cols, rows }, { cross_cols, cross_rows }, b_size);

	CUCH(cudaGetLastError());
	CUCH(cudaDeviceSynchronize()); TICK("Run cross corr: ");

	std::vector<vec2<size_t>> maxes_i = get_maxarg(cu_cross_res, cross_cols, cross_rows, b_size); TICK("Get maxarg: ");
