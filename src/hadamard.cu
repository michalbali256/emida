...
__global__ void hadamard(
	...
	esize_t batch_size)
{
	esize_t i = blockIdx.x * blockDim.x + threadIdx.x;
	
	esize_t slice_tid = i % one_size.area();
	esize_t slice_num = i / one_size.area();
	size2_t slice_pos = { slice_tid % one_size.x, slice_tid / one_size.x };
	esize_t ref_i = i % (one_size.area() * ref_slices);
	...
	esize_t num_elements = (one_size.x - 1) * 2 * one_size.y;
	mul = { mul.x / num_elements, mul.y / num_elements };
	pics[i] = mul;
}

template<typename T>
void run_hadamard(T* pics, const T* ref, const T* shx, const T* shy, size2_t one_size, esize_t ref_slices, esize_t batch_size)
{
	esize_t block_size = 1024;
	esize_t grid_size(div_up(one_size.area() * ref_slices * batch_size, block_size));
	hadamard << <grid_size, block_size >> > (pics, ref, shx, shy, one_size, ref_slices, batch_size);
}

template void run_hadamard<cufftComplex>(cufftComplex* A, const cufftComplex* B, const cufftComplex* shx, const cufftComplex* shy, size2_t one_size, esize_t ref_slices, esize_t batch_size);
template void run_hadamard<cufftDoubleComplex>(cufftDoubleComplex* A, const cufftDoubleComplex* B, const cufftDoubleComplex* shx, const cufftDoubleComplex* shy, size2_t one_size, esize_t ref_slices, esize_t batch_size);
...
template<typename T>
__global__ void finalize_fft(const T* in, T* __restrict__ out, size2_t out_size, esize_t batch_size)
{
	esize_t i = blockIdx.x * blockDim.x + threadIdx.x;
	size2_t in_size{ out_size.x + 3, out_size.y + 1 };
	
	esize_t slice_tid = i % out_size.area();
	esize_t slice_num = i / out_size.area();
...
{
	esize_t block_size = 1024;
	esize_t grid_size(div_up(out_size.area() * batch_size, block_size));
	finalize_fft <<<grid_size, block_size >>> (in, out, out_size, batch_size);
}

template void run_finalize_fft<double>(const double* in, double* out, size2_t out_size, esize_t batch_size);
template void run_finalize_fft<float>(const float* in, float* out, size2_t out_size, esize_t batch_size);
