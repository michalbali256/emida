...
TEST(cross_corr_serial, matrix_res5x5_block_3x3)
{
    std::vector<double> a =
    {
        1, 2, 3, 4,
    ...
    std::vector<double> res = cross_corr_serial(a.data(), b.data(), {4,3});
...
INSTANTIATE_TEST_SUITE_P(
    ...
    cross_corr_test,
    ::testing::Values(
        corr_param{ {3, 3}, {5, 5}, {3, 3}, "matrix_res5x5_block3x3" },
        corr_param{ {3, 3}, {5, 5}, {5, 5}, "matrix_res5x5_block5x5" },
        corr_param{ {5, 5}, {9, 9}, {5, 5}, "matrix_res9x9_block5x5" },
        corr_param{ {5, 5}, {9, 9}, {9, 9}, "matrix_res9x9_block9x9" },
        corr_param{ {96, 96}, {191, 191}, {31, 31}, "matrix_res191x191_block31x31" }
    ),
    stringer<corr_param>()
);
...
{
    double val = 1;
    std::vector<double> a(GetParam().size.area());
    for (size_t i = 0; i < a.size(); ++i)
        a[i] = val++;
    
    std::vector<double> b(GetParam().size.area());
    for (size_t i = 0; i < b.size(); ++i)
        b[i] = val++;

    std::vector<double> expected = cross_corr_serial(a.data(), b.data(), GetParam().size);

    auto cu_a = vector_to_device(a);
    auto cu_b = vector_to_device(b);
    double* cu_res = cuda_malloc<double>(GetParam().res_size.area());

    cuda_memset(cu_res, 0, GetParam().res_size.area());
    
    auto b_s = GetParam().block_size;
    run_cross_corr_opt(cu_a, cu_b, cu_res, GetParam().size, GetParam().res_size, GetParam().block_size, 1, 1);

    CUCH(cudaGetLastError());
    CUCH(cudaDeviceSynchronize());

    auto res = device_to_vector(cu_res, GetParam().res_size.area());
