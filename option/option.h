...
namespace mbas
{
	...
	{
	public:
		parsed_option(std::string name, std::vector<std::unique_ptr<parsed_param_base>> params, bool parse_ok, bool constr_ok, bool count_ok, bool unknown)
			: name_(std::move(name)), params_(std::move(params)), p_ok_(parse_ok), constr_ok_(constr_ok), count_ok_(count_ok), unknown_(unknown) {}
		...
		parsed_option(parsed_option&& rhs) noexcept
			: name_(std::move(rhs.name_)), params_(std::move(rhs.params_)), p_ok_(rhs.p_ok_), constr_ok_(rhs.constr_ok_), count_ok_(rhs.count_ok_), unknown_(rhs.unknown_) {}
	private:
		std::string name_;
	...
	{
		static bool parse(std::string value, std::string& result) { result = value; return true; }
	};
	...
	public:
		...
		parameter(const std::string name, bool mandatory, constr_t constraint) : parameter_base(name, mandatory), constraint_(constraint) {}
	...
	public:
		...
		//in the first parameter synonyms are divided with comma where one character names are treated as short options and two or more character names as long options
		option(std::string names, const  std::string description, bool optional = true) : names(construct_names_(std::move(names))), description(description), optional(optional) {}

		//adds parameter to the option with its type (VALUE_TYPE<T>), NAME and MANDATORY flag
		//the first parameter does not have name because it serves as a deduction the type T and forces user to notice that value_type<T> is needed to parse properly
		template<typename T>
		...
		option(option&& opt) noexcept
			: names(opt.names), description(opt.description), optional(opt.optional), params_(std::move(opt.params_)), mandatory_count_(opt.mandatory_count_) {}
	...
	namespace internal
	{
		...
		typename std::enable_if<I == sizeof...(Tp), void>::type
			add_param(std::tuple<Tp...>&, option&) {}

		template<size_t I = 0, typename... Tp>
		typename std::enable_if < I < sizeof...(Tp), void>::type
			add_param(std::tuple<Tp...>& t, option& opt)
		{
	...
	class command
	{
		...
		public:
			...
			template <typename T>
			option_adder& operator()(std::string names, const  std::string description, value_type<T>, std::string param_name, typename::mbas::parameter<T>::constr_t param_constraint)
			{
				owner_.add_option<T>(std::move(names), std::move(description), true, { parameter<T>(param_name,true,param_constraint) });
				return *this;
			}

			//adds option with a parameter
			template <typename T>
			option_adder& operator()(std::string names, const  std::string description, value_type<T>, std::string param_name)
			{
				owner_.add_option<T>(std::move(names), std::move(description), true, { parameter<T>(std::move(param_name)) });
				return *this;
			}
	...
	public:
		...
		template <typename... T>
		option& add_option(std::string names, std::string description, bool optional, typename::mbas::parameter_holder<T...>::type params)
		{
			options_.emplace_back(std::move(names), std::move(description), optional);
		...
		parsed_args parse(int argc, char** argv) const
		{
			return parser(this, argc, argv).parse();
		}
		...
		public:
			...
			void parse_option(size_t & i)
			{
				...
				if (first_par_parsed)
				{
					...
					{
						//a parameter was parsed, with the option name, but either the option is unknown or known flag option
					...
					add_parameter(*opt->parameters()[0], std::move(first_par), option_parse_ok, option_constraint_ok, opt_params);
					parsed_par_count = 1;
				}
				else if (!opt)
				{
					options.emplace_back(option_name, std::vector<std::unique_ptr<parsed_param_base>>{}, false, true, true, true);
					options_mapping.emplace(option_name, &options.back());
					return;
				}
				...
				while (parsed_par_count < opt->parameters().size())
				{
					...
					{
						add_parameter(*opt->parameters()[parsed_par_count], argv[i], option_parse_ok, option_constraint_ok, opt_params);
						++parsed_par_count;
					}
				}
				...
				options.emplace_back(option_name, std::move(opt_params), option_parse_ok, option_constraint_ok, count_ok, false);
				if (!option_parse_ok)
					parse_ok = false;
			...
			bool parse_short_options(char* arg, const option * & opt, std::string& option_name, std::string& first_par)
			{
				...
				while (opt && opt->parameters().size() == 0 && arg[i] != '\0')
				{
					...
					if (opt)
					{
						options.emplace_back(prev_name, std::vector<std::unique_ptr<parsed_param_base>>{}, true, true, true, false);
			...
			void add_parameter(const parameter_base& parameter, std::string value, bool& p_ok, bool& constraint_ok, std::vector<std::unique_ptr<parsed_param_base>> & opt_params) const
			{
	...
	namespace internal
	{
		...
		std::unique_ptr<parsed_param_base> parameter_base_accessor::parse(const parameter_base& parameter, const std::string parsed)
		{
